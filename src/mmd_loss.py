...
def mmd_loss_weighted(embedding, auxiliary_labels, weights_pos,
    ...
    pos_neg_mask = torch.mm(auxiliary_labels,
                            (1.0 - auxiliary_labels).t())
    neg_pos_mask = torch.mm((1.0 - auxiliary_labels),
                            auxiliary_labels.t())
    ...
    pos_kernel_mean = torch.div(
        pos_kernel_mean.sum(dim=1), weights_pos.sum())

    pos_kernel_mean = torch.div(
        (pos_kernel_mean * weights_pos.squeeze()).sum(),
        weights_pos.sum())
    ...
    neg_kernel_mean = torch.div(
        neg_kernel_mean.sum(dim=1), weights_neg.sum())

    neg_kernel_mean = torch.div(
        (neg_kernel_mean * weights_neg.squeeze()).sum(),
        weights_neg.sum())

    neg_pos_kernel_mean = kernel_mat * neg_pos_mask
    neg_pos_kernel_mean = neg_pos_kernel_mean * weights_pos.t()

    neg_pos_kernel_mean = torch.div(
        neg_pos_kernel_mean.sum(dim=1), weights_pos.sum())

    neg_pos_kernel_mean = torch.div(
        (neg_pos_kernel_mean * weights_neg.squeeze()).sum(),
        weights_neg.sum())
    ...
    pos_neg_kernel_mean = torch.div(
        pos_neg_kernel_mean.sum(dim=1), weights_neg.sum())

    pos_neg_kernel_mean = torch.div(
        (pos_neg_kernel_mean * weights_pos.squeeze()).sum(),
        weights_pos.sum())
